...
def test_cv_fold_model(session_scope_module):
    ...
    cv_fold = (
        session_scope_module.query(CVFold).filter(CVFold.event_id == event.id).all()
    )
    assert "train fold [" in repr(cv_fold[0])


@pytest.mark.parametrize(
    "backref, expected_type", [("submissions", SubmissionOnCVFold)]
)
def test_cv_fold_model_backref(session_scope_module, backref, expected_type):
    event = get_event(session_scope_module, "iris_test")
    cv_fold = (
        session_scope_module.query(CVFold).filter(CVFold.event_id == event.id).first()
    )
